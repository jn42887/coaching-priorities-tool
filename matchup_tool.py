...
readable_labels = {
    'OREB': 'Offensive Rebounding', 'DREB': 'Defensive Rebounding',
    'FTRate': 'Free Throw Rate', 'OppFTRate': 'Opponent Free Throw Rate',
    'TOVRate': 'Turnovers', 'OppTOVRate': 'Opponent Turnovers',
    'oQSQ': 'Offensive Shot Quality', 'dQSQ': 'Defensive Shot Quality',
    '3PARate': '3PA Rate', '3PARateAllowed': '3PA Rate Allowed',
    'AvgOffPace': 'Avg Off Pace', 'AvgDefPace': 'Avg Def Pace'
}
...
defensive_stats = ["dQSQ", "OppTOVRate", "DREB", "OppFTRate", "3PARateAllowed", "AvgDefPace"]
offensive_stats = [s for s in counterpart_map if s not in defensive_stats]
...
with st.sidebar:
    ...
    **Priority Score = Importance × 0.7 + Variability × 0.3**
    ...
    This is done for both your team and opponent (counterpart stat), then scaled 1–100.
...
selected_priority = {
    "Product (x)": scaled_product,
    "Weighted Average": scaled_weighted,
    "Powered Product^1.5": scaled_power
}[method]
...
for stat, counterpart in counterpart_map.items():
    if opponent == "All Teams":
        ...
        top_n = int(opponent.split()[1])
        opponents = df.groupby("Team")["NETRTG"].mean().sort_values(ascending=False).head(top_n).index
        matchup_scores[stat] = team_scores[stat] * selected_priority.loc[opponents][counterpart].mean()
    else:
...
matchup_df = pd.DataFrame({
    ...
    "Type": ["Defense" if stat in defensive_stats else "Offense" for stat in matchup_series.index]
...
color_map = {"Defense": "Blues", "Offense": "Greens"}
def highlight_by_type(row):
    cmap = color_map[row.Type]
    return [f"background-color: {cmap}" if c == row.Type else "" for c in matchup_df.columns]

st.subheader("Matchup Priority Factors")
st.markdown("Green = Offense, Blue = Defense")
styled_scores = matchup_df.style.apply(lambda x: [
    f"background-color: {{'Offense': '#c7f0c7', 'Defense': '#c7d7f0'}[x.Type] if col=='Matchup Priority Score' else ''"
...
for stat in neutral_stats:
    ...
    if stat == "AvgOffPace":
        direction, label = ("Slower" if raw_imp > 0 else "Faster"), "Pace"
    elif stat == "AvgDefPace":
        direction, label = ("Slower" if raw_imp > 0 else "Faster"), "Opp Pace"
    elif stat == "3PARate":
        direction, label = ("More" if raw_imp > 0 else "Less"), "Threes"
    elif stat == "3PARateAllowed":
        direction, label = ("More" if raw_imp > 0 else "Less"), "Opp Threes"
